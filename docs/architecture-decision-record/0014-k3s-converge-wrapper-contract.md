...
`k3s_converge` wrapper contract を導入する。初期段階では live start を担わせず、`--check-only`
の source-only/read-only probe から始めた。2026-07-01 の SwitchBot off/on 検証で、agent 側は
mount / config / unit 配置後に daemon start を行う責務が必要だと分かったため、agent play では明示的に
`k3s_converge_check_mode: start` を指定し、wrapper が `systemctl daemon-reload` と
`systemctl start k3s.service` を担う段階へ進める。
...
4. agent side の start 分離では、`xanmanning.k3s` を `k3s_state: downloaded` に寄せる。
  ...
  2026-07-01 時点では、local role として `k3s_agent_install_config` を追加し、agent play へ接続した。
  この role は public-safe default、assert task、binary link task、agent-only unit 配置、token file 配置を持つ。
  daemon lifecycle は持たず、start は `k3s_converge` wrapper だけが担う。
5. `k3s_converge` が live start / restart を担う段階では、agent play に upstream
  ...
  2026-07-01 時点で agent play の upstream role は `k3s_state: downloaded` であり、local install/config/unit/token
  配置後に `k3s_converge --start` を実行する。
6. wrapper が live start を担う段階で、post-merge apply と SwitchBot off/on を 1 セットで検証する。
7. server side は agent side の実機収束後に進める。2026-07-01 時点で `k3s-converge --node-role server`
  を source-only で許可し、server 用 local role `k3s_server_install_config` の disabled scaffold を追加した。
  upstream `xanmanning.k3s` の `k3s_state: downloaded` は binary download/upload だけなので、server 側でも
  config、optional token file、systemd unit の配置を local role に寄せる必要がある。site.yml wiring と live
  apply は別 PR / 別 acceptance set で扱う。
8. 十分に安定した後で `homecluster-k3s-converge.service` への unit 名分離を検討する。
...
agent start 実装後は live behavior が変わる。private operator workflow で post-merge apply、
active rootfs / DHCP Option 224 verifier、boot-order gate、integrated iSCSI verifier、SwitchBot off/on を
1 つの acceptance set として扱う。
